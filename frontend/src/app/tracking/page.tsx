...
const Map = dynamic(() => import("@/components/tracking/Map"), {
  ssr: false,
  loading: () => (
    <div className="h-[400px] w-full animate-pulse rounded-lg bg-slate-100 dark:bg-slate-800" />
  ),
});
...
export default function TrackingPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      console.log("Searching with file:", searchFile.name);
      // Lower threshold to 0.3 for better matching (was 0.5)
      const results = await tracksApi.searchByImage(searchFile, 5, 0.3);
      ...
      if (results.length > 0) {
        ...
        console.warn(
          "No matches found. Gallery might be empty or threshold too high."
        );
      }
    } catch (error) {
      ...
      <main className="flex-1 space-y-4 p-6">
        ...
        <div className="flex items-center justify-between rounded bg-slate-800 p-2 text-xs text-white">
              ...
              onClick={async () => {
                try {
                  const res = await fetch(
                    "http://localhost:8000/api/v1/streams/gallery"
                  );
                  const data = await res.json();
                  console.log("Gallery Info:", data);
                  alert(
                    `Gallery has ${data.gallery_size} persons. Check console for details.`
                  );
                } catch (e) {
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            {searchFile && searchResults.length === 0 && !isSearching && (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Each camera must have a different ID for tracking to
                        work properly.
                      </p>
                    </div>
                    <div className="space-y-2">
            ...
            {sources.length === 0 ? (
              <Card>
                <CardContent className="flex h-64 flex-col items-center justify-center text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Click "Add Source" to add video files or cameras
                  </p>
                </CardContent>
              </Card>
            ) : (
                ...
                <div className="space-y-2">
                  {searchResults.map((result, idx) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground">
                        Cameras: {result.track.camera_sequence.join(" -> ")}
                      </div>
                    </div>
                  ))}
                </div>
